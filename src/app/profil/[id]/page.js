...
export default function ProfilPublic() {
  ...
  const chargerProfil = async () => {
    try {
      const [profilRes, avisRes] = await Promise.all([
        api.get(`/profil/public/${id}`),
        api.get(`/avis/utilisateur/${id}`)
      ]);
      setProfil(profilRes.data.profil);
      setAnnonces(profilRes.data.annonces || []);
      setStats(avisRes.data.statistiques);
    } catch (erreur) {
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          <div className="px-6 pb-6">
            <div className="flex items-end justify-between -mt-12 mb-4">
              ...
              {utilisateur && utilisateur.id !== id && (
                <div className="mt-14">
                  <Link href={`/messages?destinataire=${id}&annonce=${annonces[0]?.id || ''}`}
              ...
              <div>
                ...
                {stats && parseInt(stats.total) > 0 && (
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="font-bold text-gray-800">{stats.moyenne}</span>
                    <span className="text-gray-400 text-sm">({stats.total} avis)</span>
                  </div>
                )}
              </div>
        ...
        <div className="flex gap-2 mb-6 bg-white rounded-2xl p-2 shadow-sm">
          {[
            ...
            { id: 'avis', label: `Avis (${stats?.total || 0})` },
            ...(utilisateur && utilisateur.id !== id ? [{ id: 'donner_avis', label: 'Laisser un avis' }] : [])
          ].map((o) => (
                ...
                {annonces.map((annonce) => (
                  <Link key={annonce.id} href={`/annonces/${annonce.id}`}>
                    <div className="border border-gray-100 rounded-xl overflow-hidden hover:border-blue-200 hover:shadow-md transition cursor-pointer">
                      <div className="h-40 bg-blue-50 relative overflow-hidden">
                        ...
                        {(annonce.statut === 'loue' || annonce.statut === 'vendu') && (
                          <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center">
                            <span className={`text-white font-black text-xl px-4 py-2 rounded-xl border-4 rotate-[-15deg] ${
                              annonce.statut === 'loue' ? 'bg-red-500 border-red-300' : 'bg-gray-800 border-gray-600'
                            }`}>
                      ...
                      <div className="p-3">
                        <h3 className="font-semibold text-gray-800 text-sm mb-1 line-clamp-1">{annonce.titre}</h3>
        ...
        {/* Avis */}
        {onglet === 'avis' && (
          <ListeAvis utilisateurId={id} />
        )}

        {/* Donner un avis */}
        {onglet === 'donner_avis' && utilisateur && utilisateur.id !== id && (
          <FormulaireAvis
