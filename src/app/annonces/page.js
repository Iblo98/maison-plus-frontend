...
import { Search, MapPin, Home, Shield, Eye, Heart, Square, Map, LayoutList } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center gap-1 bg-gray-100 rounded-xl p-1 ml-auto">
              <button onClick={() => setVueActive('liste')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition ${
                  vueActive === 'liste' ? 'bg-white text-blue-600 shadow' : 'text-gray-500'
                }`}>
                <LayoutList size={16} />
                Liste
              </button>
              <button onClick={() => setVueActive('carte')}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition ${
                  vueActive === 'carte' ? 'bg-white text-blue-600 shadow' : 'text-gray-500'
                }`}>
                <Map size={16} />
                Carte
              </button>
        ...
        {/* Vue carte */}
        {vueActive === 'carte' && !chargement && (
          <div className="mb-6">
            <CarteAnnonces annonces={annonces} />
            <p className="text-xs text-gray-400 mt-2 text-center">
              Les annonces sans coordonnées GPS ne sont pas affichées sur la carte
            </p>
          </div>
        )}
