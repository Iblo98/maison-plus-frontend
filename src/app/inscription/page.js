...
export default function Inscription() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (typeCompte === 'professionnel' && !form.nom_entreprise) {
    ...
    try {
      // Créer le compte
      await inscription({ ...form, type_compte: typeCompte });
      ...
      toast.success('Compte créé ! Vérifiez votre email et complétez votre profil.');
      router.push('/kyc');
    } catch (erreur) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4 py-8">
      <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-lg">
        ...
        <p className="text-gray-500 text-center mb-6">Rejoignez Maison+ gratuitement</p>

        {/* Choix type de compte */}
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button type="button" onClick={() => setTypeCompte('particulier')}
            className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition ${
              typeCompte === 'particulier'
                ? 'border-blue-600 bg-blue-50'
                : 'border-gray-200 hover:border-blue-300'
            }`}>
            <User size={28} className={typeCompte === 'particulier' ? 'text-blue-600' : 'text-gray-400'} />
            <span className={`font-medium text-sm ${typeCompte === 'particulier' ? 'text-blue-600' : 'text-gray-600'}`}>
              Particulier
            </span>
            <span className="text-xs text-gray-400 text-center">Personne physique</span>
          </button>

          <button type="button" onClick={() => setTypeCompte('professionnel')}
            className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition ${
              typeCompte === 'professionnel'
                ? 'border-blue-600 bg-blue-50'
                : 'border-gray-200 hover:border-blue-300'
            }`}>
            <Building size={28} className={typeCompte === 'professionnel' ? 'text-blue-600' : 'text-gray-400'} />
            <span className={`font-medium text-sm ${typeCompte === 'professionnel' ? 'text-blue-600' : 'text-gray-600'}`}>
              Professionnel
            </span>
            <span className="text-xs text-gray-400 text-center">Entreprise / Hôtel / Agence</span>
          </button>
        </div>
            ...
            <div className="space-y-4 border-t pt-4">
              ...
              <div>
                ...
                <input type="text" name="nom_entreprise" value={form.nom_entreprise} onChange={handleChange}
                  placeholder="Ex: Hôtel Azalaï" required
                  className="w-full border border-gray-300 rounded-xl px-4 py-3 outline-none focus:border-blue-500 transition"
                />
              </div>
